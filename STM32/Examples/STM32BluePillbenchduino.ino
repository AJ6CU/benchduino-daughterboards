...
// Modified July 22, 2020 to Support STM32 - Mark Hatch AJ6CU - please note that using D11 and D12 for LED's required that the console
// Output be CLosed (Serial.end(); )first since the pins default to USB.
...
// ==============================================================================
  const int PinA  = PA2;         const int PinB  = PA3;         const int PinSW = PA4;    // Rotary encoder
  const int button1 = PA5;
  const int button2 = PA6;
  const int button3 = PA7;
  const int button4 = PA8;
  const int led1 = PA9;
  const int led2 = PA10;
  const int led3 = PA11;
  const int led4 = PA12;
  const int statuspin   = PC13;
  const int beeperpin = D23; // 22;
// ==============================================================================
...
void isr ()  {
  ...
  if (interruptTime - lastInterruptTime > 20) {      // If interrupts come slower than 5ms, we assume it's good data
...
// Set pin modes
// ==============================================================================
    Serial.end();  //Needed to use pin PA11(D11) and PA12(D12). Otherwise used for comm over usb to console
...
// ==============================================================================
  ...
  payload = "";
//=== LCD SETUP =================================================================
  lcd.setBacklightPin(3,POSITIVE);  lcd.setBacklight(HIGH);  lcd.begin(16, 2);  lcd.clear();
